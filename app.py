...
.badge {background:#116699;color:#fff;padding:6px 10px;border-radius:8px;display:inline-block;}
.status {color:#fff;padding:24px;border-radius:16px;text-align:center;font-size:28px;font-weight:800;}
</style>
...
def next_folio(con):
    ...
    try:
        ...
        return f"CGTCH-25-{int(datetime.now().timestamp())%10000:04d}"
...
def add_attendee(con, data):
    ...
    token = f"TKN-{int(datetime.utcnow().timestamp())}-{np.random.randint(1000,9999)}"
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cur = con.cursor()
    cur.execute("""
        INSERT INTO attendees (folio, nombre, institucion, tipo_cuota, email, telefono, qr_token, registrado_en)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (folio, data["nombre"], data.get("institucion",""), data.get("tipo_cuota",""),
          data.get("email",""), data.get("telefono",""), token, now))
    con.commit()
    return folio, token
...
def badge(text, color=PRIMARY_COLOR):
    st.markdown(f"<span class='badge' style='background:{color}'>{text}</span>", unsafe_allow_html=True)
...
tabs = st.tabs(["📝 Registro", "📊 Reportes", "⚙️ Ajustes", "ℹ️ Ayuda"])
...
with tabs[1]:
    ...
    with c1:
        ...
        st.download_button("Descargar asistentes (CSV)", data=csv_a, file_name="asistentes.csv", mime="text/csv")
    with c2:
        ...
        st.download_button("Descargar check-ins (CSV)", data=csv_c, file_name="checkins.csv", mime="text/csv")
...
with tabs[3]:
    st.subheader("Ayuda")
    st.markdown("""
**Verificación por URL (sin cámara embebida)**  
1) En *Registro*, genera el QR de cada asistente.  
2) Con la **cámara del celular**, escanea el QR.  
3) El navegador abrirá la URL con `?token=...` y realizará el check-in.  
4) Verás pantalla **🟢 verde** si es válido, **🟡 amarillo** si ya estaba registrado en esa sede o **🔴 rojo** si no existe.
""")
